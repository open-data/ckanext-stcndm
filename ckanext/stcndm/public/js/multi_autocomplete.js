/**
 * @title WET-BOEW multi select autcomplete
 ...
 */
var componentName = "wb-multilist",
    attr = "data-multilist",
    selector = "input[" + attr + "]",
    settings = {
        separator: "; ",
	...
	populateOptions = function( input ) {
		var $input = $( input ),
		    containerClass = "wb-al-cnt",
		    sourceClass = "wb-al-src",
			$autolist = $input.next( "." + containerClass ),
			datalist = document.getElementById( input.getAttribute( attr ) ),
			options = datalist.getElementsByTagName( "option" ),
			len = options.length,
			sourceContent = "",
			$source, option, value, label, i;

		if ( $autolist.length === 0 ) {
		    $autolist = $( "<div class='" + containerClass + "' style='position:relative;'></div>" )
		        .append( "<ul role='listbox' id='wb-al-" + input.id + "' class='wb-al dropdown-menu hide' style='display:block' aria-hidden='true' aria-live='polite'></ul>" )
		        .insertAfter( $input );
		    $source =  $( "<ul class='" + sourceClass + " hide' aria-hidden='true'></ul>" )
		        .appendTo( $autolist );
		} else {
            $source = $autolist.find( "." + sourceClass).empty();
		}
		...
		for ( i = 0; i !== len; i += 1 ) {
			...
			sourceContent += "<li id='al-opt-" + input.id + "-" + i +
	...
	showOptions = function( input, value ) {
		...
		if ( value && value.length !== 0 ) {
			comparator = settings.parseRegexp.exec( value )[ 2 ].toLowerCase();
	...
	selectOption = function( input, option) {
	    var value = option + settings.separator;

        input.value = input.value ? input.value.replace( settings.parseRegexp, "$1" + value ) : value
	},
	 ...
	 */
	keyboardHandlerAutolist = function( which, link, event ) {
...
$document.on( "keydown click vclick touchstart", ".wb-al a, .wb-al span", function( event ) {
	...
	case "keydown":
		if ( !( event.ctrlKey || event.metaKey ) ) {
			return keyboardHandlerAutolist( which, link, event );
		}
		break;
	case "click":
	case "vclick":
