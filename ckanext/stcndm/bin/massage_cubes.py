...
for codeset in results['results']:
    ...
    if codeset['codeset_type'] == 'geo_level':
        geo_level_list.append({
            'label': codeset['title'],
            'value': codeset['codeset_value']
        })
...
results = rc.action.package_search(
    ...
    rows=1000)
for result in results['results']:
    if 'geodescriptor_specific_code' in result:
        continue
...
for i in range(0, 10):
    ...
    for line in query_results['results']:
        ...
        if 'geolevel_en_txtm' in line:
            result = code_lookup('geolevel_en_txtm', line, geo_level_list)
            if result:
                line_out['geo_level_codes'] = result
